...
SKILL_PATH = os.path.expanduser(
    "~/.config/opencode/skills/businessbot/SKILL.md"
)

# High-quality free fallback models on OpenRouter
FALLBACK_MODELS = [
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are NexSupport, a professional and empathetic AI assistant "
            "specialised in customer service and SME business support. "
            "Help users with orders, refunds, account issues, and business "
            "operations. Be concise, warm, and solution-focused."
        )
...
def _extract_retry_after(e: Exception) -> float:
    """Extracts retry wait time from the exception object or its text representation."""
    default_wait = 5.0
    try:
        # Check standard OpenAI API error body dictionary
    ...
    try:
        # Regular expression search inside the stringified exception
        err_str = str(e)
...
class NexSupportBot:
    ...
    def get_closing(self) -> str:
        return (
            "Thank you for chatting with me! If you ever need help again, "
    ...
    def chat(self, message: str) -> str:
        ...
        # Build the model list to try, starting with the primary configured model
        ...
        while current_model_idx < len(models_to_try):
            ...
            while retries <= max_retries_per_model:
                try:
                    ...
                    # If we fell back successfully, notify and update standard model
                    ...
                    # Check for rate limiting status or texts
                    ...
                    if is_rate_limit:
                        ...
                        if retries <= max_retries_per_model:
                            # If retry wait time is short/reasonable, wait and retry this model
                            if retry_after <= 10.0:
                                if self.status_callback:
                                ...
                                # For longer rate limit cooldowns, immediately switch to the next fallback model
                                if self.status_callback:
                                    self.status_callback(
                        ...
                        # For other provider/server errors, try fallback model immediately
                        ...
                        if is_provider_err:
                            ...
                            # Critical or user-configuration errors (e.g. 401 Unauthorized, wrong credentials)
                            # shouldn't trigger fallback; we return them immediately.
